...
//		public NewtonVR.NVRHand rightHandForSimpleIK;
		...
		public NVRHandData leftHandData;
		public NVRHandData rightHandData;
...
//		private NewtonVR.NVRHand rightHand;
		...
		//private bool leftHandHoldButtonDown = false;
		//private bool leftHandHoldButtonUp   = false;
		//private bool rightHandHoldButtonDown = false;
		//private bool rightHandHoldButtonUp   = false;
		...
		// For HumanNaviOnline experiment
		private bool isStatedRecording = false;
...
//				Debug.LogWarning("LeftHand.name=" + this.leftHand.name);
		...
		void Update()
		{
			try
			{
				...
				if (this.interruptedReason != string.Empty && this.step != Step.WaitForNextSession)
				{
					...
					this.SendPanelNotice("Failed\n" + interruptedReason.Replace('_', ' '), this.fontSize, PanelNoticeStatus.Red);
					this.TimeIsUp();
				}

				this.leftHandData .UpdateModeratorData();
				this.rightHandData.UpdateModeratorData();
				...
				//if(!HumanNaviConfig.Instance.configInfo.photonServerMachine && this.receivedMessageMap[MsgIamReady])
				//{
				//	this.punRpcManager.ForwardNVRHandData(this.leftHandData);
				//	this.punRpcManager.ForwardNVRHandData(this.rightHandData);
				//}
				...
				switch (this.step)
				{
					case Step.Initialize:
					{
						if (this.isCompetitionStarted)
						{
							this.PreProcess();
							this.step++;
//							Debug.LogWarning("Step.Initialize");
						}
					...
					case Step.WaitForStart:
					{
						...
						{
//							Debug.LogWarning("Step.WaitForStart1");

							//if (this.IsPlaybackInitialized() && (!HumanNaviConfig.Instance.configInfo.photonServerMachine || this.IsConnectedToRos()))
							if (!HumanNaviConfig.Instance.configInfo.photonServerMachine || this.IsConnectedToRos())
							{
//								Debug.LogWarning("Step.WaitForStart2");
								this.step++;
							}
						}
					...
					case Step.SessionStart:
					{
						...
						//this.SendPanelNotice("Session start!", this.fontSize, PanelNoticeStatus.Green);
...
//						this.JudgeGraspingObject();
		...
		void LateUpdate()
		{
			this.preLeftHandHoldButtonDown  = this.leftHandData .HoldButtonDownForModerator;
			this.preLeftHandHoldButtonUp    = this.leftHandData .HoldButtonUpForModerator;
			this.preRightHandHoldButtonDown = this.rightHandData.HoldButtonDownForModerator;
			this.preRightHandHoldButtonUp   = this.rightHandData.HoldButtonUpForModerator;
		}
...
//			Debug.LogWarning("rosConnections.Length=" + this.rosConnections.Length);
		...
		private void TimeIsUp()
		{
			...
			this.SendPanelNotice("Time is up", this.fontSize, PanelNoticeStatus.Red);
		...
		public void StartRecording()
		{
			...
			StartCoroutine(this.WaitStartRecording());			
		}
		...
		private IEnumerator WaitStartRecording()
		{
			while (!this.IsPlaybackInitialized())
			{
		...
		private void StartPlaybackRecord()
		{
			if (HumanNaviConfig.Instance.configInfo.playbackType == WorldPlaybackCommon.PlaybackTypeRecord)
			{
				...
				this.isStatedRecording = this.playbackRecorder.Record();
				this.recordButton.interactable = false;
				this.stopButton.interactable = true;

				//if (!isStarted) { SIGVerseLogger.Warn("Cannot start the world playback recording"); }
				if (!this.isStatedRecording) { SIGVerseLogger.Warn("Cannot start the world playback recording"); }
			}
		}
		...
		private void CheckGraspOfObject(NVRHandData hand)
		{
//			if(!hand.isEnable){ return; }

//			if (!HumanNaviConfig.Instance.configInfo.photonServerMachine)
			{
				//if(hand.IsRight && hand.HoldButtonDownForModerator)
				//{
				//	SIGVerseLogger.Error("hand.IsRight && hand.HoldButtonDown");
				//}
					...
					if (hand.CurrentlyInteractingTag == TagNameOfGraspables)
					{
						if (this.IsTargetObject(hand.CurrentlyInteractingName))
						{
							if (!this.isTargetAlreadyGrasped)
							{
								...
								this.punRpcManager.ForwardTargetObjectGrasp(hand.CurrentlyInteractingName);
							}
						}
						else
						{
							this.punRpcManager.ForwardWringObjectGrasp(hand.CurrentlyInteractingName);
							...
							//if (!this.isTargetAlreadyGrasped)
							{
								...
								//	this.SendPanelNotice("Wrong object is grasped", this.fontSize, PanelNoticeStatus.Red);
		...
		public void TargetObjectGrasp(string graspedObjectName)
		{
			SIGVerseLogger.Info("Target object is grasped" + "\t" + this.GetElapsedTimeText());
		...
		public void WrongObjectGrasp(string graspedObjectName)
		{
			if (!this.alreadyGraspedObjects.Contains(graspedObjectName))
			{
				SIGVerseLogger.Info("Wrong object is grasped [new]" + "\t" + this.GetElapsedTimeText() + "\t" + this.GetElapsedTimeText());
				...
				this.alreadyGraspedObjects.Add(graspedObjectName);
			}
			else
			{
				SIGVerseLogger.Info("Wrong object is grasped [already grasped]" + "\t" + this.GetElapsedTimeText() + "\t" + this.GetElapsedTimeText());
			...
			this.RecordEventLog("Object_Is_Grasped" + "\t" + "Wrong_Object" + "\t" + graspedObjectName);

			this.SendRosHumanNaviMessage("Wrong_object_grasp", "");
		}

		public void SetNVRHandData(
			...
			)
		{
			if (isRight)
			{
				this.rightHandData.HoldButtonDownForModerator = holdButtonDown;
				this.rightHandData.HoldButtonUpForModerator = holdButtonUp;
				this.rightHandData.HoldButtonPressed = holdButtonPressed;
				this.rightHandData.IsRight = isRight;
				this.rightHandData.CurrentlyInteractingName = currentlyInteractingName;
				this.rightHandData.CurrentlyInteractingTag = currentlyInteractingTag;
				this.rightHandData.IsInteracting = isInteracting;
			}
			else if (isLeft)
			{
				this.leftHandData.HoldButtonDownForModerator = holdButtonDown;
				this.leftHandData.HoldButtonUpForModerator = holdButtonUp;
				this.leftHandData.HoldButtonPressed = holdButtonPressed;
				this.leftHandData.IsRight = isRight;
				this.leftHandData.CurrentlyInteractingName = currentlyInteractingName;
				this.leftHandData.CurrentlyInteractingTag = currentlyInteractingTag;
				this.leftHandData.IsInteracting = isInteracting;
			}
			else
			{
				Debug.LogWarning("SetNVRHandData: left or right?");
			}
		}

		private string GetGraspingObjectId(NVRHandData hand)
		{
			string graspingObject = "";

//			if(!hand.isEnable){ return graspingObject; }
			...
			if (hand.HoldButtonPressed)
			{
				if (hand.IsInteracting)
				{
					if (hand.CurrentlyInteractingTag == TagNameOfGraspables)
					{
						graspingObject = hand.CurrentlyInteractingName;
					}
				}
			}
		...
		private void CheckGraspingStatus(NVRHandData hand)
		{
//			if(!hand.isEnable){ return; }

			if (hand.HoldButtonDownForModerator)
			{
//				SIGVerseLogger.Info("HandInteraction" + "\t" + "HoldButtonDown" + "\t" + hand.name + "\t" + this.GetElapsedTimeText());
			...
			if (hand.HoldButtonUpForModerator)
			{
				string objectInhand = "";
...
//					SIGVerseLogger.Info("HandInteraction" + "\t" + "HoldButtonUp" + "\t" + hand.name + "\t" + this.GetElapsedTimeText());
		...
		public void TargetPlacedOnDestination()
		{
			...
			this.SendPanelNotice("Task succeeded", this.fontSize, PanelNoticeStatus.Green);
		...
		public void OnGiveUp()
		{
			if (this.isDuringSession)
			{
				...
				this.SendPanelNotice("Give up", this.fontSize, PanelNoticeStatus.Red);
		...
		public void OnReachMaxWrongObjectGraspCount()
		{
			...
			this.SendPanelNotice("Reach max wrong object grasp count", this.fontSize, PanelNoticeStatus.Red);
		...
		public IEnumerator DisplayEndMessage()
		{
			...
			this.SendPanelNotice(endMessage, this.fontSize, PanelNoticeStatus.Blue, false);
		}
		...
		private IEnumerator ClearRosConnections()
		{
			...
			foreach (IRosConnection rosConnection in this.rosConnections)
			{
//				Debug.LogWarning("rosConnection.ToString()="+rosConnection.ToString());
				rosConnection.Clear();
			}

			SIGVerseLogger.Info("Clear ROS connections. con num=" + this.rosConnections.Length);
		}
