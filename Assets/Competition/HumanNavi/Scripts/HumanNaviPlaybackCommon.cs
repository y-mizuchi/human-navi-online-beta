	...
	public class HumanNaviPlaybackCommon : TrialPlaybackCommon
	{
		...
		public void SetPlaybackTargets()
		{
			...
			if (HumanNaviConfig.Instance.configInfo.playbackType == WorldPlaybackCommon.PlaybackTypeRecord)
			{
				...
				// Avatar
				Transform avatar2 = GameObject.FindGameObjectWithTag("Avatar2").transform;

				Transform[] avatarTransforms2 = avatar2.GetComponentsInChildren<Transform>(true);

				foreach (Transform avatarTransform in avatarTransforms2)
				{
					bool isTagergetTransform = true;
					foreach (string keyword in KeywordsOfAvatarPartsPathToIgnore)
					{
